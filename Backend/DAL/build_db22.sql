...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[TbItems]') AND [c].[name] = N'FabricId');
IF @var2 IS NOT NULL EXEC(N'ALTER TABLE [TbItems] DROP CONSTRAINT ' + @var2 + ';');
ALTER TABLE [TbItems] DROP COLUMN [FabricId];
...
IF @var3 IS NOT NULL EXEC(N'ALTER TABLE [TbStyles] DROP CONSTRAINT ' + @var3 + ';');
...
IF @var4 IS NOT NULL EXEC(N'ALTER TABLE [TbSliders] DROP CONSTRAINT ' + @var4 + ';');
...
IF @var5 IS NOT NULL EXEC(N'ALTER TABLE [TbOrderItems] DROP CONSTRAINT ' + @var5 + ';');
...
IF @var9 IS NOT NULL EXEC(N'ALTER TABLE [TbItems] DROP CONSTRAINT ' + @var9 + ';');
...
IF @var10 IS NOT NULL EXEC(N'ALTER TABLE [TbCategories] DROP CONSTRAINT ' + @var10 + ';');
...
IF @var11 IS NOT NULL EXEC(N'ALTER TABLE [TbCartItem] DROP CONSTRAINT ' + @var11 + ';');
...
IF @var12 IS NOT NULL EXEC(N'ALTER TABLE [TbOrderItems] DROP CONSTRAINT ' + @var12 + ';');
...
IF @var13 IS NOT NULL EXEC(N'ALTER TABLE [TbImages] DROP CONSTRAINT ' + @var13 + ';');
...
IF @var15 IS NOT NULL EXEC(N'ALTER TABLE [TbChanges] DROP CONSTRAINT ' + @var15 + ';');
