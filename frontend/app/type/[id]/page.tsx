...
export default function TypePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true)
        const [itemsData, categoriesData, typesData] = await Promise.all([
        ...
        // Normalize types and find current type
        const normalizedTypes = (Array.isArray(typesData) ? typesData : [])
          .map((t) => ({
            id: t.id ?? "",
            name: t.name ?? "",
        ...
        const matchedType = normalizedTypes.find(
          (t) => t.id === typeId || t.name?.toLowerCase() === typeId.toLowerCase()
        )
        setTypeInfo(matchedType || null)

        const targetTypeId = matchedType?.id ?? typeId

        // Normalize items and filter by type
        ...
        const typeProducts = normalized.filter((p) => p.typeId === targetTypeId)
  ...
  // Get unique category IDs present in the type-filtered products
            ...
            <div className="relative z-10 p-6 md:p-8">
              ...
              <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-[11px] tracking-[0.25em] text-[#7B3F32]/70 font-semibold uppercase">
                  {language === "ar" ? "تسوق حسب النوع" : "Shop By Type"}
                </p>
              </div>
              ...
              {allTypes.length > 1 && (
                <div className="flex flex-wrap items-center gap-2 mt-5 pt-4 border-t border-[#7B3F32]/10">
                  <span className="text-[10px] text-[#7B3F32]/50 uppercase tracking-[0.2em] font-bold mr-2">
                    {language === "ar" ? "أنواع أخرى:" : "Other Types:"}
                  </span>
                  {allTypes
              ...
              {loading ? (
                ...
                <div className="rounded-3xl border border-white/60 bg-white/75 backdrop-blur-sm p-10 text-center shadow-[0_10px_40px_rgba(0,0,0,0.06)]">
                  ...
                  <p className="text-[#7B3F32]/75 mb-6">
                    {language === "ar"
                      ? "لا توجد منتجات بهذا النوع حالياً. جرّب تغيير الفلاتر أو تصفح أنواع أخرى."
                      : "No products match this type yet. Try adjusting your filters or explore other types."}
                  </p>
