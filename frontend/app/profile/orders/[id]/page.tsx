...
function statusBadge(status: string, language: string) {
  ...
  switch (normalized) {
    ...
    case "processing":
      return <Badge className="bg-cyan-100 text-cyan-700 border border-cyan-200 hover:bg-cyan-200">{language === "ar" ? "جاري المعالجة" : "Processing"}</Badge>
    case "shipped":
      return <Badge className="bg-indigo-100 text-indigo-700 border border-indigo-200 hover:bg-indigo-200">{language === "ar" ? "تم الشحن" : "Shipped"}</Badge>
    case "delivered":
      return <Badge className="bg-emerald-100 text-emerald-700 border border-emerald-200 hover:bg-emerald-200">{language === "ar" ? "تم التوصيل" : "Delivered"}</Badge>
    case "cancelled":
      return <Badge className="bg-rose-100 text-rose-700 border border-rose-200 hover:bg-rose-200">{language === "ar" ? "ملغي" : "Cancelled"}</Badge>
    case "refunded":
      return <Badge className="bg-violet-100 text-violet-700 border border-violet-200 hover:bg-violet-200">{language === "ar" ? "مرتجع" : "Refunded"}</Badge>
    default:
...
function paymentStatusBadge(status: string, language: string) {
  ...
  switch (normalized) {
    case "paid":
      return <Badge variant="secondary" className="bg-emerald-100 text-emerald-700 border-emerald-200">{language === "ar" ? "مدفوع" : "Paid"}</Badge>
    case "refunded":
      return <Badge variant="secondary" className="bg-violet-100 text-violet-700 border-violet-200">{language === "ar" ? "مرتجع" : "Refunded"}</Badge>
    case "failed":
      return <Badge variant="secondary" className="bg-rose-100 text-rose-700 border-rose-200">{language === "ar" ? "فشل" : "Failed"}</Badge>
    case "pending":
    default:
      return <Badge variant="secondary" className="bg-amber-100 text-amber-700 border-amber-200">{language === "ar" ? "معلق" : "Pending"}</Badge>
  }
}
