...
const getStatusMeta = (status: string, language: "ar" | "en") => {
  ...
  if (normalized.includes("deliver")) {
    return {
      ...
      className: "inline-flex items-center justify-center rounded-xl px-2.5 py-1 text-xs font-semibold bg-green-500 text-white border-2 border-green-600",
    }
  }
  if (normalized.includes("ship")) {
    return {
      label: language === "ar" ? "تم الشحن" : "Shipped",
      className: "inline-flex items-center justify-center rounded-xl px-2.5 py-1 text-xs font-semibold bg-indigo-500 text-white border-2 border-indigo-600",
    }
  }
  if (normalized.includes("process")) {
    return {
      label: language === "ar" ? "قيد المعالجة" : "Processing",
      className: "inline-flex items-center justify-center rounded-xl px-2.5 py-1 text-xs font-semibold bg-blue-500 text-white border-2 border-blue-600",
    }
  }
  if (normalized.includes("cancel")) {
    return {
      label: language === "ar" ? "ملغي" : "Cancelled",
      className: "inline-flex items-center justify-center rounded-xl px-2.5 py-1 text-xs font-semibold bg-red-500 text-white border-2 border-red-600",
    }
  }
  if (normalized.includes("refund")) {
    return {
      label: language === "ar" ? "مرتجع" : "Refunded",
      className: "inline-flex items-center justify-center rounded-xl px-2.5 py-1 text-xs font-semibold bg-purple-500 text-white border-2 border-purple-600",
    }
  }

  return {
    label: language === "ar" ? "قيد الانتظار" : "Pending",
    className: "inline-flex items-center justify-center rounded-xl px-2.5 py-1 text-xs font-semibold bg-yellow-500 text-white border-2 border-yellow-600",
  }
}
